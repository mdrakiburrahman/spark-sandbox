...
SUITE_PALETTE = ["#118DFF", "#12239E", "#E66C37", "#6B007B", "#118DFF", "#D13438", "#FFB900", "#107C10", "#00B294", "#5C2D91"]
...
def draw_card(fig, x, y, w, h, value: str, label: str, accent: str = PBI["accent"]) -> None:
    ...
    fig.add_artist(bar)
    fig.text(x + 0.012, y + h * 0.55, value, fontsize=26, fontweight="bold", color=PBI["ink"], transform=fig.transFigure)
    fig.text(x + 0.012, y + h * 0.18, label, fontsize=10, color=PBI["muted"], transform=fig.transFigure)
...
# Header band
fig.text(0.01, 0.965, "<Report Title>", fontsize=22, fontweight="bold", color=PBI["ink"])
fig.text(0.01, 0.94, "<Report subtitle / one-line description>", fontsize=10, color=PBI["muted"])
...
fail_by_suite = fact_failed.merge(dim_suite[["suite_id", "suite_name"]], on="suite_id", how="left").groupby([pd.to_datetime(fact_failed["failure_date"]), "suite_name"]).size().rename("fails").reset_index()
...
for i, suite in enumerate(top_suites):
    ...
    ax1.plot(s["failure_date"], s["fails"], marker="o", linewidth=2, color=SUITE_PALETTE[i % len(SUITE_PALETTE)], label=suite[:30])
...
fail_by_owner = fact_failed.merge(dim_owner[["owner_email", "owner_display_name"]], on="owner_email", how="left").groupby([pd.to_datetime(fact_failed["failure_date"]), "owner_display_name"]).size().rename("fails").reset_index()
...
for i, owner in enumerate(top_owners):
    ...
    ax2.plot(s["failure_date"], s["fails"], marker="o", linewidth=2, color=SUITE_PALETTE[i % len(SUITE_PALETTE)], label=str(owner)[:25])
...
d = fact_dur.groupby(pd.to_datetime(fact_dur["duration_date"])).agg(p50=("p50_ms", "mean"), p95=("p95_ms", "mean")).reset_index()
...
ax4.axis("off")
worst = fact_mtbf.nlargest(10, "failure_rate_pct")[["suite_name", "failure_count", "failure_rate_pct", "mtbf_days"]]
worst.columns = ["Suite", "Fails", "Fail %", "MTBF days"]
tbl = ax4.table(cellText=worst.values, colLabels=worst.columns, loc="center", cellLoc="left", colLoc="left")
