...
object LakeDeltaVacuumMetadata extends DeltaVacuumMetadata {
  ...
  val desiredDeltaTableConfigs: Array[DesiredDeltaTableConfig] = Array(
    ...
    DesiredDeltaTableConfig(database = "data_ops_inventory_db",       tableNameOrPrefix = "dbt_node_executions",                 isPrefix = false,    skipVacuum = false,   skipOptimize = false,    skipPurge = false,     purgePartitionColumn = SortableColumnNames.YEAR_MONTH_EVENT.toString,         purgePartitionColumnDateType = DateTypes.YearMonth,             numPartitionsToRetain = 7,                zOrderColumns = Array("resource_type", "name", "unique_id")),
    ...
    // reddit_db
    // =====================================================================
    DesiredDeltaTableConfig(database = "reddit_db",                  tableNameOrPrefix = "*",                                   isPrefix = true,     skipVacuum = false,   skipOptimize = false,    skipPurge = true,      purgePartitionColumn = "",                                                    purgePartitionColumnDateType = null,                            numPartitionsToRetain = Int.MaxValue,     zOrderColumns = Array.empty[String]),
  )
}
// @formatter:on
