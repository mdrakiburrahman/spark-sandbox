...
/opt/spark/bin/spark-sql \
    ...
    --conf "spark.sql.repl.eagerEval.truncate=0" \
     -e "SELECT request_body FROM data_ops_inventory_db.http_dumper_plugin LIMIT 1" --silent 2>/dev/null | grep -v -E '^$|^::|^request_body$' > /workspaces/spark-sandbox/projects/spark-scala/.temp/openlineage/lineage-from-spark-custom-plugin.json
```
