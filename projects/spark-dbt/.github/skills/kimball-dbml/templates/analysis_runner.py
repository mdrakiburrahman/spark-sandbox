...
def profile_table(con: duckdb.DuckDBPyConnection, table_path: Path) -> dict:
    ...
    profile["sample"] = (
        con.execute(f"SELECT * FROM {src} LIMIT {SAMPLE_ROWS}").fetchdf()
    )
    ...
    for col in cols_to_profile:
        ...
        if any(t in col_type.upper() for t in ("DATE", "TIMESTAMP", "TIME")):
            try:
                rng = con.execute(
                    f"SELECT MIN({qcol}), MAX({qcol}) FROM {src}"
                ).fetchone()
...
def find_fk_candidates(profiles: list[dict]) -> list[dict]:
    ...
    for col_name, occurrences in by_name.items():
        if len(occurrences) < 2:
            continue
        sorted_by_distinct = sorted(
            occurrences, key=lambda x: x[1].get("distinct", 0), reverse=True
        )
...
def render_markdown(
    db_root: Path, profiles: list[dict], fk_candidates: list[dict]
) -> str:
    ...
    lines.append("")
    lines.append(
        "> Generated by `kimball-dbml/templates/analysis_runner.py`. "
        "**Not committed** — gitignored under `<dbt-project>/tmp/`."
    )
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    for prof in profiles:
        lines.append(
            f"| `{prof['name']}` | {prof['row_count']:,} | {len(prof['columns'])} |"
        )
    lines.append("")
    ...
    for prof in profiles:
        ...
        for col in prof["columns"]:
            ...
            lines.append(
                ...
                f"{distinct_str} | {' · '.join(notes) if notes else ''} |"
            )
        lines.append("")
    ...
    if fk_candidates:
        ...
        lines.append("")
        lines.append(
            "Columns appearing in more than one table — likely FK relationships. "
            "Highest-distinct-count table is shown as the inferred parent."
        )
        lines.append("")
        ...
        for fk in fk_candidates:
            lines.append(
                f"| `{fk['column']}` | `{fk['parent']}` | "
                f"{fk['parent_distinct']:,} | `{fk['child']}` | "
                f"{fk['child_distinct']:,} |"
            )
        lines.append("")
    ...
    lines.append("")
    lines.append(
        "_Next: use these findings to draft the bus matrix (see "
        "`templates/bus_matrix_template.md`) and the dbml (see `templates/dbml-skeleton.dbml`)._"
    )
    lines.append("")
...
def main() -> int:
    ...
    if not tables:
        print(
            f"No Delta tables found under {args.db} "
            "(looking for subdirs containing _delta_log/).",
            file=sys.stderr,
        )
        return 1
