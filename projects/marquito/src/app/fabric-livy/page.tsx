...
import { ConnectionPhase, LivyConfig, KpiResult, DeltaCommitEntry, UberLineage } from '@/lib/livy/types';
...
export default function FabricLivyPage() {
  ...
  const config: LivyConfig | null = jwt && workspaceId && lakehouseId
  ...
  if (mode === 'choose') {
    return (
      ...
      >
        ...
        <div style={{ display: 'grid', gap: '16px', maxWidth: '600px', width: '100%' }}>
          {/* Option 1: Query Livy */}
          <div
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '10px' }}>
              <div style={{ width: '36px', height: '36px', borderRadius: '8px', backgroundColor: isDark ? 'rgba(0,120,212,0.15)' : 'rgba(0,120,212,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
  ...
  if (mode === 'connect' && connectionPhase === 'idle') {
    return (
      ...
      >
        <h1 style={{ fontSize: '24px', fontWeight: 600, color: isDark ? '#FAF9F8' : '#323130', marginBottom: '8px' }}>
          Connect to Fabric Livy
        </h1>
        <p style={{ fontSize: '14px', color: isDark ? '#A19F9D' : '#605E5C', marginBottom: '24px', textAlign: 'center', maxWidth: '520px', lineHeight: '1.5' }}>
          Provide your Azure credentials to connect to a Fabric Lakehouse via the Livy API.
        </p>

        <div style={{ maxWidth: '520px', width: '100%', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {/* JWT */}
          <div>
            <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', fontWeight: 600, color: isDark ? '#D2D0CE' : '#323130', marginBottom: '6px' }}>
          ...
          <button
            ...
            style={{
              ...
              gap: '8px',
              padding: '10px 20px',
              backgroundColor: jwt && workspaceId && lakehouseId ? '#0078D4' : isDark ? '#484644' : '#C8C6C4',
              color: '#FFFFFF',
              border: 'none',
              borderRadius: '6px',
              fontSize: '14px',
              fontWeight: 600,
              fontFamily: "'Segoe UI', sans-serif",
              cursor: jwt && workspaceId && lakehouseId ? 'pointer' : 'not-allowed',
          ...
          <button
            onClick={() => { setMode('choose'); setConnectionPhase('idle'); }}
            style={{
              padding: '8px 16px',
